#!/bin/bash
# =============================================================================
# Deploy: Service 3 — Inventory (Cloud Run)
...
export SERVICE_NAME="service-3-inventory-cloudrun"
...
# Internal Nginx DNS — used by this Cloud Run service to call back to Service 2 (GKE)
# *.k8s.api.internal resolves to the internal Nginx IP (10.10.1.51) via Cloud DNS
export ORDERS_API_CALLBACK_URL="http://service-2-orders-api.k8s.api.internal"
...
# --vpc-egress=all-traffic     → All outbound traffic routed through VPC so it can
...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --set-env-vars="ORDERS_API_CALLBACK_URL=${ORDERS_API_CALLBACK_URL}" \
  --min-instances=0 \
  --max-instances=5 \
  --memory=256Mi \
  --cpu=1 \
...
success "Service 3 (Inventory Cloud Run) deployed successfully."
echo ""
...
echo "  Internal DNS   : http://service-3-inventory-cloudrun.api.internal"
echo "  Callback URL   : ${ORDERS_API_CALLBACK_URL}/inventory-callback"
